...
namespace UtilityWpf.ViewModel
{
    ...
    {
        public static void ReactToChanges(this InteractiveCollectionBase col, SHDOObject so)
        {
         ...
         .Subscribe(b =>
         {
            ...
            so.WhenPropertyChanged(_ => _.IsChecked).StartWith(new PropertyValue<SHDOObject, bool?>(so, so.IsChecked)).Subscribe(_ =>
              {
                  if (_.Value != null)
                  {
